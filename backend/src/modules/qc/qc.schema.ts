...
export const CreateQCSchema = z.object({
  type: z.enum(QCType, { error: 'Type harus: INCOMING, IN_PROCESS, atau FINAL' }),
  result: z.enum(QCResult, { error: 'Result harus: PASS, FAIL, atau CONDITIONAL' }),
  rawMaterialLotId: z.string().uuid('Lot ID tidak valid').optional(),
  batchId: z.string().uuid('Batch ID tidak valid').optional(),
  notes: z.string().optional(),
  metrics: z.record(z.string(), z.unknown()).optional(), // Flexible JSON for measurement data
});
